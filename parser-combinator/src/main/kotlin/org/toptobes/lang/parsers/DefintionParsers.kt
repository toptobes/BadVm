...
val variableByteDef = contextual { ctx ->
    (ctx parse -byteType) ?: error("Not a var byte declaration")

    val name = (ctx parse sequence(
        optionalWhitespace, !str("@"), identifier,
    )..{ it[2] }) ?: fail("Error with var byte declaration")

    ctx.ifParseable(equals) {
        val bytes = (ctx parse any(singleByteList, multiByteList, byteString)) ?: fail("Error with = var byte definition")
    ...
    fail("Var byte definition doesn't have equals nor size")
}

val variableWordDef = contextual { ctx ->
    (ctx parse -wordType) ?: error("Not a var word declaration")

    val name = (ctx parse sequence(
        optionalWhitespace, !str("@"), identifier,
    )..{ it[2] }) ?: fail("Error with var word declaration")

    ctx.ifParseable(equals) {
        val words = (ctx parse any(singleWordList, multiWordList, wordString)) ?: fail("Error with = var word definition")
    ...
    fail("Var word definition doesn't have equals nor size")
}

// -- CONST DEFINITIONS --

val constByteDef = contextual { ctx ->
    (ctx parse (str("const") then -byteType)) ?: error("Not a const byte declaration")
    (ctx parse !str("$"))

    val name = (ctx parse identifier) ?: fail("Error with const byte name declaration")

    (ctx parse equals) ?: fail("Const byte definition doesn't have an equals")

    val byte = (ctx parse byte) ?: fail("Const byte is not assigned to a proper byte")

    success(Const8Definition(name, byte))
}

val constWordDef = contextual { ctx ->
    (ctx parse (str("const") then -wordType)) ?: error("Not a const word declaration")
    (ctx parse !str("$"))

    val name = (ctx parse identifier) ?: fail("Error with const word name declaration")

    (ctx parse equals) ?: fail("Const word definition doesn't have an equals")

    val word = (ctx parse word) ?: fail("Const word is not assigned to a proper word")
